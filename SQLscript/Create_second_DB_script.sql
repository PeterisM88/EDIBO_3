...
(
    ...
    Title NVARCHAR(160)  NOT NULL,
    fkArtistId INTEGER  NOT NULL,
    CONSTRAINT FK_ArtistIdAlbums2 FOREIGN KEY (fkArtistId) REFERENCES artists (pkArtistId) 
        ON DELETE NO ACTION ON UPDATE NO ACTION);



 insert into Albums2(Title, fkArtistId)  values("due ",2 );
...
(
    ...
    Bytes INTEGER,
    UnitPrice NUMERIC(10,2)  NOT NULL,
    FOREIGN KEY (pkAlbumId) REFERENCES Albums2 (pkAlbumId) 
        ON DELETE NO ACTION ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS Playlists_track
(

    FOREIGN KEY (PlaylistId) REFERENCES Playlists (pkPlaylistid) 
        ON DELETE NO ACTION ON UPDATE NO ACTION,
    FOREIGN KEY (TrackId) REFERENCES Tracks (pkTrackId) 
        ON DELETE NO ACTION ON UPDATE NO ACTION
);
